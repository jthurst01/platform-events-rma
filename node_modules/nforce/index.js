...
var BLITZ_AUTH_ENDPOINT = 'https://login-blitz03.soma.salesforce.com/services/oauth2/authorize';
...
var BLITZ_LOGIN_URI     = 'https://na7-blitz03.soma.salesforce.com/services/oauth2/token';
var ENVS               = ['sandbox', 'production', 'blitz'];
...
var Connection = function(opts) {
  ...
  if(!_.isString(this.environment) || _.indexOf(ENVS, this.environment) === -1) {
    throw new Error('invalid environment, only ' + ENVS.join(' and ') + ' are allowed, not this ' + this.environment);
  }
...
Connection.prototype.getAuthUri = function(opts) {
  ...
  } else if(self.environment == 'blitz') {
    endpoint = this.blitzAuthEndpoint;
  } else {
...
Connection.prototype.authenticate = function(data, callback) {
  ...
  if(self.environment == 'sandbox') {
    opts.uri = this.testLoginUri;
  } else if(self.environment == 'blitz') {
    opts.uri = this.blitzLoginUri;
  } else {
    opts.uri = this.loginUri;
  }
...
Connection.prototype.revokeToken = function(data, callback) {
  ...
  } else if(this.environment === 'blitz') {
    opts.uri = 'https://login-blitz03.soma.salesforce.com/services/oauth2/revoke';
  } else {
