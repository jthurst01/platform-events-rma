...
******/
	...
	this.version= "0.7.2.2";
		...
		this.default_settings= {
            ...
            ,right_toolbar: ""      //awaite: added for putting options in the toolbar right justified
            ...
            ,sfdc_id: ""            //the id of the class, page or component that is being edited by the given editor
            ,sfdc_name: ""          //the name of the class, page or component that is being edited by the given editors
            ,viewstateHrefUri: ""   //the uri for viewstate
		...
		this.advanced_buttons = [
				...
				['search', 'search', 'show_search', false],
				['go_to_line', 'go_to_line', 'go_to_line', false],
				['undo', 'undo', 'undo', true],
				['redo', 'redo', 'redo', true],
...
EditAreaLoader.prototype ={
	...
	init : function(settings){
        ...
        if(settings["right_toolbar"].length>0) {
            settings["right_tab_toolbar"]= settings["right_toolbar"].replace(/ /g,"").split(",");
        }
	...
	delete_instance : function(id){
		...
		var span= $("EditAreaArroundInfos_"+id);
		if(span)
		...
		if(iframe){
			iframe.parentNode.removeChild(iframe);  //delete iframe;
			try {
	...
	start : function(id){
		...
		if(!$("EditAreaArroundInfos_"+id) && (editAreas[id]["settings"]["debug"] || editAreas[id]["settings"]["allow_toggle"]))
		{
        ...
        if(area["settings"]["right_tab_toolbar"] != null) {
            html_toolbar_content+= "<span style=\"position:absolute;right:10px;top:3px;vertical-align:middle\">";
            for(var j=0; j<area["settings"]["right_tab_toolbar"].length; j++) {
                html_toolbar_content+= this.get_control_html(area["settings"]["right_tab_toolbar"][j], area["settings"]["language"]); 
            }
            html_toolbar_content+= "</span>";
        }
		...
		area.textarea=$(area["settings"]["id"]);
		...
		setAttr(content, "frameBorder", "0"); // IE
	...
	toggle_off : function(id){
		...
		{	
			...
			setAttr(editAreas[id]["textarea"], "wrap", "off");	// for Firefox	
			...
			//W-1591173: Adding a NULL check to shelter FF iframe deletion bug. 
			var iframe= $("frame_"+id);
			if(iframe)
	...
	toggle_on : function(id){
		...
		if(window.frames["frame_"+id])
		{
			...
			// set to good size
			this.set_editarea_size_from_textarea(id, $("frame_"+id));
			editAreas[id]["textarea"].style.display="none";			
			$("frame_"+id).style.display="inline";
	...
	get_button_html : function(id, imgClass, exec, isFileSpecific, baseURL) {
		if(!baseURL)
		...
		html+= '<img id="' + id + '" src="/s.gif" title="{$' + id + '}" class="' + imgClass + '_icon" onmouseover="editArea.switchClass(this,\'' + imgClass + '_iconOn\');" onmouseout="editArea.restoreClass(this);" onmousedown="editArea.restoreAndSwitchClass(this,\'' + imgClass + '_icon\');" /></a>';
		return html;
	},
	...
	reset : function(e){
		...
		for(var i in editAreas){			
			...
			if(window.frames["frame_"+i] && is_child && editAreas[i]["displayed"]==true){
				...
				var exec= 'window.frames["frame_'+ i +'"].editArea.textarea.value= $("'+ i +'").value;';
		...
		for(var i in editAreas){
			...
			if(is_child)
			{
				if(window.frames["frame_"+i] && editAreas[i]["displayed"]==true)
					$(i).value= window.frames["frame_"+ i].editArea.textarea.value;
    ...
    // allow to set the value of the editarea and set the new position
    setValueWithPos : function(id, new_val, startSelect, endSelect){
        if(window.frames["frame_"+id] && editAreas[id]["displayed"]==true){        
	        if(editAreaLoader.nav['isIE']) {
				var parentScrollTop = parent.document.documentElement.scrollTop;
				var parentScrollLeft = parent.document.documentElement.scrollLeft;
	        	var scrollTop= window.frames["frame_"+ id].$("result").scrollTop;
				var scrollLeft= window.frames["frame_"+ id].$("result").scrollLeft;
				
				window.frames["frame_"+ id].editArea.textarea.value= new_val;    
	            window.frames["frame_"+ id].editArea.area_select(startSelect, endSelect - startSelect);
	            parent.document.documentElement.scrollTop = parentScrollTop;
	            parent.document.documentElement.scrollLeft = parentScrollLeft;
	            window.frames["frame_"+ id].$("result").scrollTop= scrollTop;
				window.frames["frame_"+ id].$("result").scrollLeft= scrollLeft; 
			} else {
				window.frames["frame_"+ id].editArea.textarea.value= new_val;    
	           	window.frames["frame_"+ id].editArea.area_select(startSelect, endSelect - startSelect);            
			}
	...
	hide : function(id){
		if($(id) && !this.hidden[id])
		{
			this.hidden[id]= new Object();
			this.hidden[id]["selectionRange"]= this.getSelectionRange(id);
			if($(id).style.display!="none")
			{
				this.hidden[id]["scrollTop"]= $(id).scrollTop;
				this.hidden[id]["scrollLeft"]= $(id).scrollLeft;
			}
