...
******/
	...
	EditArea.prototype.init= function(){
		...
		var num_lines = 0;
		if (textarea.value) {
			num_lines = this.textarea.value.split("\n").length;
		}
		if (num_lines > 0) {
			this.addLineNumbers(1, num_lines);
		}
		...
		if($("redo") != null)
			this.switchClassSticky($("redo"), 'redo_disabled_icon', true);
        ...
        if(this.nav['isSafari']){
            if( this.nav['isSafari'] < 3.2 || this.nav['isSafari'] > 4.5)
                this.editor_area.style.position= "absolute";
            if( this.nav['isSafari'] < 5)
                this.textarea.style.marginLeft="-3px";
            if( this.nav['isSafari'] > 4.5) {
                // fixes ghosting when a string that's longer-than-editor-width is pasted
	...
	EditArea.prototype.manage_size= function(onlyOneTime){
		...
		if(editAreas[this.id]["displayed"]==true && this.textareaFocused)
		{
			...
			{
				var start = this.line_number + 1;
				var end = this.last_selection["nb_line"] + 100;
				this.addLineNumbers(start, end);
			}
	...
	EditArea.prototype.addLineNumbers = function(start, end) {
		var newLines = "";
		for (var i = start; i <= end; i++) {
			newLines += "<div id='line_" + i + "'>" + i + "</div>";
			this.line_number++;
		}
		var destDiv = $("line_number");
