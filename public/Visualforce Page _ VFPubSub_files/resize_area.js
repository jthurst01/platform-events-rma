...
/*	EditAreaLoader.prototype.full_screen= function(id, mode){
		if(mode=="on")	{
			//$("frame_"+id).parentNode= document;
			document.body.appendChild($("frame_"+id));
			$("frame_"+id).style.position= "absolute";			
			$("frame_"+id).style.top="0px";
			$("frame_"+id).style.left="0px";
			$("frame_"+id).style.width= "100%";
			$("frame_"+id).style.height= "100%";
		}else{
