	...
	EditArea.prototype.check_line_selection= function(timer_checkup){
		...
		if(!this.smooth_selection && !this.do_highlight){
			...
			// if selection change
				...
				new_top = this.get_line_pos_top(infos["line_start"]);
	...
	EditArea.prototype.get_line_pos_top = function(start_line) {
		var elem = $("line_" + start_line);
		var pos_top = 0;
		...
		if (elem) {
			...
			pos_top = this.lineHeight * (start_line - 1);
		}
		return pos_top;
	};
	...
	EditArea.prototype.getIESelection= function(){
		if(!need_createRange) return;  // VF_MOD - IE < 9 (W-937066)	
	...
	EditArea.prototype.displayToCursorPosition= function(id, start_line, cur_pos, lineContent, no_real_move){
		...
		var posTop = this.get_line_pos_top(start_line);
